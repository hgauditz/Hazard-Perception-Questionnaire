...
def info_data(df):
    ...
    # inspect age
    age_group = list(df.groupby('group')['age'].apply(list).values)
    ...
    print('\nage per group (control, stroke): ' +str(age_group))
...
def spearman_correlation(df_input):
    ...
    all_total = stats.spearmanr(df['age'], df['total']) #small effect
    all_behaviour = stats.spearmanr(df['age'], df['behaviour']) #no effect
    all_emotion = stats.spearmanr(df['age'], df['emotion']) #small effect
    stroke_total = stats.spearmanr(df_stroke['age'], df_stroke['total']) #no effect
    stroke_behaviour = stats.spearmanr(df_stroke['age'], df_stroke['behaviour']) #no effect
    stroke_emotion = stats.spearmanr(df_stroke['age'], df_stroke['emotion']) #large effect
    control_total = stats.spearmanr(df_control['age'], df_control['total']) #small effect
    control_behaviour = stats.spearmanr(df_control['age'], df_control['behaviour']) #no effect
    control_emotion = stats.spearmanr(df_control['age'], df_control['emotion']) #small effect
